...
export default function Cart({ onClose }) {
  ...
  const CartTitle =
    Object.keys(globalDefaults.cart).length > 0 ? (
      <div className={classes.div_title_cart}>
  ...
  const ItemsInCart =
    Object.keys(globalDefaults.cart).length > 0 ? (
      Object.keys(globalDefaults.cart).map((key) => (
        <div key={"cart_" + key}>
          {isMobile && (
            <div className={classes.img_item_cart}>
              <ItemImg data={globalData.Items[key]} onClick={onClose} />
            </div>
          )}
          <div className={classes.div_items_cart_child}>
            {!isMobile && (
              <div className={classes.img_item_cart}>
                <ItemImg data={globalData.Items[key]} onClick={onClose} />
              </div>
            ...
            <InputField
              ...
              onIncrement={() =>
                handleQtyChange(
                  key,
                  globalDefaults.cart[key].qty,
                  globalDefaults.cart[key].qty + 1
                )
              }
              onDecrement={() =>
                handleQtyChange(
            ...
            />
            <div>
              {globalData.Items[key].price} {currency}
            </div>
      ...
      <div className={classes.div_empty_cart}>{t("cart_empty")}</div>
    )
  ...
  const SubmitOrder =
    Object.keys(globalDefaults.cart).length > 0 ? (
      <div className={classes.div_submit_cart_container}>
        <div className={classes.div_cart_total}>
          {t("sum")} {TotalSum} {currency}
        </div>
        <div className={classes.div_cart_button_wrapper}>
          <Button onClick={onClose}>{t("cart_сancel")}</Button>
        </div>
        <div className={classes.div_cart_button_wrapper}>
          <NavLink
            to="/Submit"
  ...
  return (
    <div className={classes.div_items_cart_container}>
      {!isMobile && CartTitle}
      <div className={classes.div_items_cart_parent}>{ItemsInCart}</div>
      {SubmitOrder}
    </div>
  )
}